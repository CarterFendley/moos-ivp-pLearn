...
# Copy template directory to export dir
printf "\nCopying model files... "
cp -r "$MYDIR/_export_template" "$EXPORT_TO_DIR"
printf "Done!\n"

# Copy model files
printf "\nCopying model files... "
cp "$MODEL_PATH/../environment.py" "$EXPORT_TO_DIR/environment.py"
cp "$MYDIR/_constants_wrapper.py" "$EXPORT_TO_DIR/Constants.py"
...
cp -r "$MYDIR/../../src" "$EXPORT_TO_DIR/src"
...
echo "=        WARNING           ="
echo "============================"
echo "This script's tests are not exhaustive! If pLearn is changed"
...
# 7) A !!MAJOR!! assumption is that the vehicle running the behavior is BOTH on the red team AND felix
